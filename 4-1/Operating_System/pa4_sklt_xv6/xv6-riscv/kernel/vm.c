...
mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  for(;;){
    ...
    *pte = PA2PTE(pa) | perm | PTE_V;
    ...
    pages[idx].pagetable = pagetable;
    pages[idx].vaddr     = (char*)a;
...
handle_page_fault(pagetable_t pagetable, uint64 va)
{
  ...
  uint64 entry = *pte;
  uint64 flags = PTE_FLAGS(entry);

  if(!pte) return -1;

  // user 페이지 체크
  if(!(flags & PTE_U)) return -1;
  ...
  uint64 pa = (uint64)mpage;

  // swap 오프셋이 상위 비트에 남아 있으면 꺼내 놓고…
  int blkno = (int)(entry >> 10);
  *pte = PA2PTE(pa) | flags | PTE_V;

  // TLB 플러시
  sfence_vma(pagetable, va);

  // 예전에 swap된 페이지면 디스크에서 복원
  if(blkno > 0){
    swapread(pa, blkno);
    swap_used[blkno] = 0;
  }
