...
for size in data_sizes:
    ...
    # ORQT 초기화 및 업데이트 후 메모리 사용량 측정
    orqt = ORQT(sample_data)
    updates = [(i, sample_data[i] + 1) for i in range(size)]
    measure_orqt_update_time(orqt, updates)
    orqt_memory_usage.append(measure_memory_usage())
    
    # 배열 초기화 및 업데이트 후 메모리 사용량 측정
    array_data = sample_data[:]
    measure_array_update_time(array_data, updates)
    array_memory_usage.append(measure_memory_usage())
    
    # 연결 리스트 초기화 및 업데이트 후 메모리 사용량 측정
    linked_list = LinkedList()
    for value in sample_data:
        linked_list.insert(value)
    measure_linked_list_update_time(linked_list, updates)
    linked_list_memory_usage.append(measure_memory_usage())
    
    # 이진 트리 초기화 및 업데이트 후 메모리 사용량 측정
    binary_tree = BinaryTree()
    root = None
    for value in sample_data:
        root = binary_tree.insert(root, value)
    measure_binary_tree_update_time(root, updates)
...
plt.title('Memory Usage Comparison After Updates')
