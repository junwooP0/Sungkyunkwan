...
def main():
    ...
    while round <= 13:
        ...
        if choice == '1':
            ...
            while recoll_count < 2:
                ...
                if reroll_choice == '1':
                    while True:
                        ...
                        # 입력이 1-5의 범위를 벗어나거나 5개를 초과하는 경우 오류 메시지 출력
                        if all(i.isdigit() and 1 <= int(i) <= 5 for i in reroll_index) and len(reroll_index) <= 5:
                            # 중복된 값을 제거하고 인덱스를 0부터 시작하도록 변환
                            reroll_index = list(set(int(i) - 1 for i in reroll_index))
                            break           ## 입력에서 중복된 인덱스를 하나의 인덱스로 취급하도록 하려면 reroll_index를 **집합(set)**으로 변환하여
                                            ## 중복된 인덱스를 제거한 후 다시 리스트로 변환하면 됩니다. 이렇게 하면 중복된 값은 자동으로 제거됩니다.
                        
                        ## reroll_index = [int(i) - 1 for i in reroll_index.split()]
                        ## # 입력이 0-4의 범위를 벗어나거나 5개를 초과하는 경우 오류 메시지 출력
                        ## if all(0 <= i < 5 for i in reroll_index) and len(reroll_index) <= 5:
                        ##     break
                        else:
            ...
            # 재굴림 횟수가 2회인 경우 더이상 재굴림을 할 수 없음을 알림        
            if recoll_count == 2:
